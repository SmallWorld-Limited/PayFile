<?php
    ...
    class sage_management extends db_connect {
        
        // Upload Sage file
        public function upload_sage_file($target_file_type, $file_reference, $tmp_path, $original_name, $delimiter = ',', $has_headers = 1) {
            $conn = $this->connect();
            $user_id = $_SESSION["bt_user_id"] ?? 0;
            ...
            try {
                ...
                $upload_dir = '../sage_uploads/';
                if (!file_exists($upload_dir)) {
                    mkdir($upload_dir, 0777, true);
                }
                
                // Generate unique filename
                $stored_name = 'sage_' . time() . '_' . preg_replace('/[^a-zA-Z0-9\.\-]/', '_', $original_name);
                $stored_path = $upload_dir . $stored_name;
                
                // Move uploaded file
                if (!move_uploaded_file($tmp_path, $stored_path)) {
                    throw new Exception("Failed to save uploaded file");
                }
                
                // Insert into sage_uploads table
                $upload_stmt = $conn->prepare("
                    INSERT INTO sage_uploads (original_filename, stored_filename, file_path, 
                ...
                $upload_stmt->bind_param('ssssii', $original_name, $stored_name, $stored_path, 
                                    $file_type, $file_size, $user_id);
                
                if (!$upload_stmt->execute()) {
                    throw new Exception("Failed to save upload record: " . $upload_stmt->error);
                }
                
                $sage_id = $conn->insert_id;
                $upload_stmt->close();
                
                // Parse file and extract data
                $lines = file($stored_path, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                ...
                $update_stmt = $conn->prepare("UPDATE sage_uploads SET total_records = ? WHERE id = ?");
                $update_stmt->bind_param('ii', $total_records, $sage_id);
                $update_stmt->execute();
                $update_stmt->close();
                
                // Save individual records to sage_data table
                $record_index = 0;
                foreach ($lines as $line_number => $line) {
                    if ($has_headers && $line_number == 0) {
                    ...
                    $data_stmt = $conn->prepare("
                        INSERT INTO sage_data (sage_upload_id, record_index, data_json, status)
                        VALUES (?, ?, ?, 'pending')
                    ");
                    
                    $record_index++;
                    $data_stmt->bind_param('iis', $sage_id, $record_index, $data_json);
                    $data_stmt->execute();
                    $data_stmt->close();
                }
                
                // Update status to processing
                $status_stmt = $conn->prepare("UPDATE sage_uploads SET status = 'processing' WHERE id = ?");
                $status_stmt->bind_param('i', $sage_id);
                $status_stmt->execute();
                $status_stmt->close();
                ...
                return [
                    ...
                    'message' => "Sage file uploaded successfully. $total_records records found."
                ];
        ...
        // Get Sage data preview
        // public function get_sage_preview($sage_id, $limit = 5) {
        //     $conn = $this->connect();
            
        //     $stmt = $conn->prepare("
        //         SELECT record_index, data_json
        //         FROM sage_data
        //         WHERE sage_upload_id = ?
        //         ORDER BY record_index
        //         LIMIT ?
        //     ");
            
        //     $stmt->bind_param('ii', $sage_id, $limit);
        //     $stmt->execute();
        //     $result = $stmt->get_result();
        ...
        // Get file type schema for mapping
        public function get_file_schema($file_type_id) {
            $conn = $this->connect();
        ...
        public function check_reference_exists($file_reference) {
            $conn = $this->connect();
            
            $stmt = $conn->prepare("SELECT id FROM file_batches WHERE reference_no = ?");
            $stmt->bind_param('s', $file_reference);
            $stmt->execute();
            $stmt->store_result();
    ...
    class template_manager extends db_connect {
        ...
        public function get_all_templates() {
            ...
            $sql = "
                SELECT 
                    ...
                    ft.delimiter,
                    COUNT(fs.id) as field_count,
                    (SELECT COUNT(*) FROM file_batches WHERE file_type_id = ft.id) as usage_count
                FROM file_types ft
                LEFT JOIN file_schema fs ON ft.id = fs.file_type_id
                WHERE ft.is_active = 1
                GROUP BY ft.id
        ...
        public function get_template_sample($template_id) {
            $samples = [
                1 => [ // Payment File
                    'header' => '0;TEST_13;MWK;5000.00;0005',
                    'body' => '1;0001;MWK;0013007800002;Malawi Vulnerability Assessment Committee;1000.00;UNDP;4610002;903962;58301610;615023;SBICMWM0;9100001187829;CC001;;058SC2200657843;PM1Nodate',
                    'description' => 'Standard payment file format'
                ],
                2 => [ // Remittance File
                    'header' => '0;RBM_09.09.2025;6272978.00;0010',
                    'body' => '2;0001;0013007800002;MVAC;MWK;0014000360008;MWK;937189.00;MRA;09.09.2025;09.09.2025;200TRF2200578990;55001001',
                    'description' => 'Remittance file without PRN'
                ],
                3 => [ // Remittance with PRN
                    'header' => '0;LAG21-09.09.2025;10000.00;0004',
                    'body' => '2;0001;0013006161243;Salaries Pool Account;MWK;0014000360008;MWK;1000.00;MRA Revenue;09.09.2025;09.09.2025;271TRF2100675570;271001011;0000010001001',
                    'description' => 'Remittance file with Payment Reference Number'
                ],
                4 => [ // Foreign Payment
                    'header' => '0;WTC01-31.01.2023;USD;5000000;0002',
                    'body' => '1;0001;USD;13006161244;(ORT) MG Other Recurrent Expenditure A/C;2000000;Two Million USD Only;SMARTECH INTERNATIONAL...',
                    'description' => 'Foreign currency payment file'
                ],
                5 => [ // Salary Payment
                    'header' => '0;12SC;140000.00;C;0013007800016;12SC2100537593;Test Salary for 12SC;30.05.2025;0007',
                    'body' => '1;SBICMWM0;9100001187829;12SC210053759300001;MAILOSI MODESTER;Standard Bank;049;SAL;20000.0',
                    'description' => 'Salary payment file'
                ]
            ];
            
            return isset($samples[$template_id]) ? $samples[$template_id] : null;
        }
